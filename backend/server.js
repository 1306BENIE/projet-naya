...
app.use("/api/manager", managerRouter); // Route manager après CORS
  ...
  try {
    ...
    if (!validatePhone(phone)) {
      return res.status(400).json({
        message:
          "Numéro invalide. Format attendu: 10 chiffres après 225 (ex: 2250700000000)",
      });
    }
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    if (!phone || !validatePhone(phone)) {
      errors.push(
        "Numéro invalide. Format: 10 chiffres après 225 (ex: 2250700000000)"
      );
    }
    ...
    if (email) {
      if (!/^\S+@\S+\.\S+$/.test(email)) {
        ...
        if (existingEmail)
          errors.push("Cet email est déjà utilisé pour ce rôle");
      }
    }
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    const normalizePhone = (phone) => {
      ...
      if (
        cleaned.length === 10 &&
        ["01", "05", "07"].includes(cleaned.substring(0, 2))
      ) {
        return "225" + cleaned;
      }
      if (
        ...
        ["01", "05", "07"].includes(cleaned.substring(3, 5))
      ) {
        return cleaned;
      }
      return null;
    ...
    if (!user) {
      return res
        .status(401)
        .json({ message: "Identifiants incorrects ou rôle invalide" });
    }
...
app.post("/api/commandes", protect, async (req, res) => {
  try {
    ...
    const { expedition, destination, colis, acceptCGU } = req.body;
...
app.get("/api/calculate-delivery-price", async (req, res) => {
  ...
  try {
    ...
    const [originGeocode, destGeocode] = await Promise.all([
      client.geocode({
        params: { address: origin + ", Abidjan, Côte d'Ivoire", key: apiKey },
      }),
      client.geocode({
        params: {
          address: destination + ", Abidjan, Côte d'Ivoire",
          key: apiKey,
        },
      }),
    ]);
    ...
    if (!originLocation || !destLocation) {
      return res
        .status(400)
        .json({ error: "Impossible de localiser une des adresses" });
    }
